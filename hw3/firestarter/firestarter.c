...
// TODO: Scale loop with 5000 processes
    ...
    for (i_trial=0; i_trial < n_trials; i_trial++) {
        ...
        //percent burn
        prob_spread[i_trial] = prob_min + (double)i_trial * prob_step;
        percent_burned[i_trial]=0.0;
        
        for (i_prob = 0 ; i_prob < n_probs; i_prob++) {
            
            //burn until fire is gone
            burn_until_out(forest_size,forest,prob_spread[i_trial],
                forest_size/2,forest_size/2);
            percent_burned[i_trial]+=get_percent_burned(forest_size,forest);
            
        }
        percent_burned[i_trial]/=n_probs;
        // print output
        printf("%lf , %lf\n",prob_spread[i_trial],
            percent_burned[i_trial]);
